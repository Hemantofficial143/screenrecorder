...
var addHostEventListeners = (elm, hostRef, listeners, attachParentListeners) => {
  if (BUILD.hostListener && listeners) {
    ...
    listeners.map(([flags, name, method2]) => {
      const target = BUILD.hostListenerTarget ? getHostListenerTarget(elm, flags) : elm;
      const handler = hostListenerProxy(hostRef, method2);
...
var getElement = (ref2) => BUILD.lazyLoad ? getHostRef(ref2).$hostElement$ : ref2;
var createEvent = (ref2, name, flags) => {
...
var safeCall = (instance, method2, arg) => {
  if (instance && instance[method2]) {
    try {
...
var setValue = (ref2, propName, newVal, cmpMeta) => {
  ...
  const elm = BUILD.lazyLoad ? hostRef.$hostElement$ : ref2;
          ...
          set(newValue) {
            if (BUILD.isDev) {
              ...
              if (
                ...
                (ref2.$flags$ & 8) === 0 && // the member is a prop
...
More information: https://stenciljs.com/docs/properties#prop-mutability`);
        ...
        Object.defineProperty(prototype, memberName, {
          value(...args) {
            const ref2 = getHostRef(this);
            return ref2.$onInstancePromise$.then(() => ref2.$lazyInstance$[memberName](...args));
          }
        });
      }
    });
    ...
    if (BUILD.style && Cstr.style) {
      ...
      if (!styles.has(scopeId2)) {
        ...
        if (!BUILD.hydrateServerSide && BUILD.shadowDom && BUILD.shadowDomShim && cmpMeta.$flags$ & 8) {
          style = await import("./shadow-css-MOXECRIX.js").then((m) => m.scopeCss(style, scopeId2, false));
        }
...
var getHostRef = (ref2) => hostRefs.get(ref2);
...
var loadModule = (cmpMeta, hostRef, hmrVersionId) => {
  ...
  return /* webpackInclude: /\.entry\.js$/ */ /* webpackExclude: /\.system\.entry\.js$/ */ /* webpackMode: "lazy" */ globImport_entry_js(`./${bundleId}.entry.js${BUILD.hotModuleReplacement && hmrVersionId ? "?s-hmr=" + hmrVersionId : ""}`).then((importedModule) => {
    if (!BUILD.hotModuleReplacement) {
...
var findPlayer = (ref2, interval = 300, maxRetries = 10) => {
  const el = isInstanceOf(ref2, HTMLElement) ? ref2 : getElement(ref2);
...
var createDispatcher = (ref2) => (prop, value) => {
  const el = isInstanceOf(ref2, HTMLElement) ? ref2 : getElement(ref2);
...
var getRegistrant = (ref2) => isInstanceOf(ref2, HTMLElement) ? ref2 : getElement(ref2);
function withComponentRegistry(ref2, name) {
  const registryId = Symbol("vmRegistryId");
  const registrant = getRegistrant(ref2);
...
function withComponentRegistrar(player) {
  ...
  function onRegister(e) {
    ...
    createStencilHook(ref2, void 0, () => onDeregister(registrant));
  }
  createStencilHook(player, () => {
...
function isComponentRegistered(ref2, name) {
  var _a2;
  const registrant = getRegistrant(ref2);
...
function getPlayerFromRegistry(ref2) {
  const registrant = getRegistrant(ref2);
  return registrant[PLAYER_KEY];
}
function getComponentFromRegistry(ref2, name) {
  var _a2, _b2;
  const registrant = getRegistrant(ref2);
...
function watchComponentRegistry(ref2, name, onChange) {
  var _a2;
  return __awaiter$y(this, void 0, void 0, function* () {
    const player = yield findPlayer(ref2);
    const disposal = new Disposal();
    const registry2 = getRegistrant(ref2)[REGISTRY_KEY];
    function listener(e) {
...
var usePlayerContext = (ref2, props, updater, playerRef2) => __awaiter$x(void 0, void 0, void 0, function* () {
  const player = playerRef2 !== null && playerRef2 !== void 0 ? playerRef2 : yield findPlayer(ref2);
...
var createProviderDispatcher = (ref2) => (prop, value) => {
  const el = isInstanceOf(ref2, HTMLElement) ? ref2 : getElement(ref2);
...
function withProviderConnect(ref2) {
  const connectEvent = buildProviderConnectEvent(PROVIDER_CONNECT_EVENT, ref2);
  createStencilHook(ref2, () => {
    getElement(ref2).dispatchEvent(connectEvent);
  });
}
...
var Audio = class extends HTMLElement {
  ...
  render() {
    return (
      ...
      // @ts-ignore
      h2("vm-file", { noConnect: true, willAttach: this.willAttach, crossOrigin: this.crossOrigin, preload: this.preload, disableRemotePlayback: this.disableRemotePlayback, mediaTitle: this.mediaTitle, viewType: ViewType.Audio, ref: (el) => {
...
var CaptionControl = class extends HTMLElement {
  ...
  render() {
    ...
    return h2(Host, { hidden: !this.canToggleCaptionVisibility }, h2("vm-control", { label: this.i18n.captions, keys: this.keys, hidden: !this.canToggleCaptionVisibility, pressed: this.isTextTrackVisible, onClick: this.onClick.bind(this) }, h2("vm-icon", { name: this.isTextTrackVisible ? this.showIcon : this.hideIcon, library: this.icons }), h2("vm-tooltip", { hidden: this.hideTooltip, position: this.tooltipPosition, direction: this.tooltipDirection }, tooltipWithHint)));
  }
  static get watchers() {
    return {
...
var Controls = class extends HTMLElement {
  ...
  render() {
    return h2(Host, { video: !this.isAudioView }, h2("div", { style: Object.assign(Object.assign({}, this.getPosition()), { flexDirection: this.direction, alignItems: this.align === "center" ? "center" : `flex-${this.align}`, justifyContent: this.justify }), class: {
      controls: true,
    ...
    }, onMouseEnter: this.onStartInteraction.bind(this), onMouseLeave: this.onEndInteraction.bind(this), onTouchStart: this.onStartInteraction.bind(this), onTouchEnd: this.onEndInteraction.bind(this) }, h2("slot", null)));
  }
  static get watchers() {
    return {
...
var CurrentTime = class extends HTMLElement {
  ...
  render() {
    return h2("vm-time", { label: this.i18n.currentTime, seconds: this.currentTime, alwaysShowHours: this.alwaysShowHours });
  }
  static get style() {
...
var Dailymotion = class extends HTMLElement {
  ...
  render() {
    return h2("vm-embed", { embedSrc: this.embedSrc, mediaTitle: this.mediaTitle, origin: this.getOrigin(), params: this.buildParams(), decoder: decodeQueryString, preconnections: this.getPreconnections(), onVmEmbedMessage: this.onEmbedMessage.bind(this), onVmEmbedSrcChange: this.onEmbedSrcChange.bind(this), ref: (el) => {
      this.embed = el;
    } });
  }
  static get watchers() {
...
var DefaultControls = class extends HTMLElement {
  ...
  buildAudioControls() {
    return h2("vm-controls", { fullWidth: true }, h2("vm-playback-control", { tooltipDirection: "right" }), h2("vm-volume-control", null), !this.isLive && h2("vm-current-time", null), this.isLive && h2("vm-control-spacer", null), !this.isLive && h2("vm-scrubber-control", null), this.isLive && h2("vm-live-indicator", null), !this.isLive && h2("vm-end-time", null), !this.isLive && h2("vm-settings-control", { tooltipDirection: "left" }), h2("div", { style: { marginLeft: "0", paddingRight: "2px" } }));
  }
  buildMobileVideoControls() {
    return h2(Fragment, null, h2("vm-scrim", { gradient: "up" }), h2("vm-controls", { pin: "topLeft", fullWidth: true, activeDuration: this.activeDuration, waitForPlaybackStart: this.waitForPlaybackStart, hideWhenPaused: this.hideWhenPaused }, h2("vm-control-spacer", null), h2("vm-volume-control", null), !this.isLive && h2("vm-caption-control", null), !this.isLive && h2("vm-settings-control", null), this.isLive && h2("vm-fullscreen-control", null)), h2("vm-controls", { pin: "center", justify: "center", activeDuration: this.activeDuration, waitForPlaybackStart: this.waitForPlaybackStart, hideWhenPaused: this.hideWhenPaused }, h2("vm-playback-control", { style: { "--vm-control-scale": "1.3" } })), !this.isLive && h2("vm-controls", { pin: "bottomLeft", fullWidth: true, activeDuration: this.activeDuration, waitForPlaybackStart: this.waitForPlaybackStart, hideWhenPaused: this.hideWhenPaused }, h2("vm-control-group", null, h2("vm-current-time", null), h2("vm-control-spacer", null), h2("vm-end-time", null), h2("vm-fullscreen-control", null)), h2("vm-control-group", { space: "top" }, h2("vm-scrubber-control", null))));
  }
  buildDesktopVideoControls() {
    return h2(Fragment, null, this.theme !== "light" && h2("vm-scrim", { gradient: "up" }), h2("vm-controls", { fullWidth: true, pin: "bottomRight", activeDuration: this.activeDuration, waitForPlaybackStart: this.waitForPlaybackStart, hideWhenPaused: this.hideWhenPaused, hideOnMouseLeave: this.hideOnMouseLeave }, !this.isLive && h2("vm-control-group", null, h2("vm-scrubber-control", null)), h2("vm-control-group", { space: this.isLive ? "none" : "top" }, h2("vm-playback-control", { tooltipDirection: "right" }), h2("vm-volume-control", null), !this.isLive && h2("vm-time-progress", null), h2("vm-control-spacer", null), !this.isLive && h2("vm-caption-control", null), this.isLive && h2("vm-live-indicator", null), h2("vm-pip-control", null), !this.isLive && h2("vm-settings-control", null), h2("vm-fullscreen-control", { tooltipDirection: "left" }))));
  }
  render() {
    if (this.isAudioView)
...
var DefaultSettings = class extends HTMLElement {
  ...
  buildPlaybackRateSubmenu() {
    if (this.playbackRates.length <= 1 || !this.canSetPlaybackRate) {
      return h2("vm-menu-item", { label: this.i18n.playbackRate, hint: this.i18n.normal });
    }
    const formatRate = (rate) => rate === 1 ? this.i18n.normal : `${rate}`;
    return h2("vm-submenu", { label: this.i18n.playbackRate, hint: formatRate(this.playbackRate) }, h2("vm-menu-radio-group", { value: `${this.playbackRate}`, onVmCheck: this.onPlaybackRateSelect.bind(this) }, this.playbackRates.map((rate) => h2("vm-menu-radio", { label: formatRate(rate), value: `${rate}` }))));
  }
  ...
  buildPlaybackQualitySubmenu() {
    ...
    if (this.playbackQualities.length <= 1 || !this.canSetPlaybackQuality) {
      return h2("vm-menu-item", { label: this.i18n.playbackQuality, hint: (_a2 = this.playbackQuality) !== null && _a2 !== void 0 ? _a2 : this.i18n.auto });
    }
    ...
    return h2("vm-submenu", { label: this.i18n.playbackQuality, hint: this.playbackQuality }, h2("vm-menu-radio-group", { value: this.playbackQuality, onVmCheck: this.onPlaybackQualitySelect.bind(this) }, this.playbackQualities.map((quality) => h2("vm-menu-radio", { label: quality, value: quality, badge: getBadge(quality) }))));
  }
  ...
  buildTextTracksSubmenu() {
    ...
    if (this.textTracks.length <= 1 || !this.canSetTextTrack) {
      return h2("vm-menu-item", { label: this.i18n.subtitlesOrCc, hint: (_b2 = (_a2 = this.textTracks[this.currentTextTrack]) === null || _a2 === void 0 ? void 0 : _a2.label) !== null && _b2 !== void 0 ? _b2 : this.i18n.none });
    }
    return h2("vm-submenu", { label: this.i18n.subtitlesOrCc, hint: this.isTextTrackVisible ? (_c = this.textTracks[this.currentTextTrack]) === null || _c === void 0 ? void 0 : _c.label : this.i18n.off }, h2("vm-menu-radio-group", { value: `${!this.isTextTrackVisible ? -1 : this.currentTextTrack}`, onVmCheck: this.onTextTrackSelect.bind(this) }, [h2("vm-menu-radio", { label: this.i18n.off, value: "-1" })].concat(this.textTracks.map((track, i2) => h2("vm-menu-radio", { label: track.label, value: `${i2}` })))));
  }
  ...
  buildAudioTracksMenu() {
    ...
    if (this.audioTracks.length <= 1 || !this.canSetAudioTrack) {
      return h2("vm-menu-item", { label: this.i18n.audio, hint: (_b2 = (_a2 = this.audioTracks[this.currentAudioTrack]) === null || _a2 === void 0 ? void 0 : _a2.label) !== null && _b2 !== void 0 ? _b2 : this.i18n.default });
    }
    return h2("vm-submenu", { label: this.i18n.audio, hint: (_c = this.audioTracks[this.currentAudioTrack]) === null || _c === void 0 ? void 0 : _c.label }, h2("vm-menu-radio-group", { value: `${this.currentAudioTrack}`, onVmCheck: this.onAudioTrackSelect.bind(this) }, this.audioTracks.map((track, i2) => h2("vm-menu-radio", { label: track.label, value: `${i2}` }))));
  }
  render() {
    return h2("vm-settings", { pin: this.pin }, this.buildAudioTracksMenu(), this.buildPlaybackRateSubmenu(), this.buildPlaybackQualitySubmenu(), this.isVideoView && this.buildTextTracksSubmenu(), h2("slot", null));
  }
  static get watchers() {
    return {
...
var DefaultUI = class extends HTMLElement {
  ...
  render() {
    return h2("vm-ui", null, !this.noClickToPlay && h2("vm-click-to-play", null), !this.noDblClickFullscreen && h2("vm-dbl-click-fullscreen", null), !this.noCaptions && h2("vm-captions", null), !this.noPoster && h2("vm-poster", null), !this.noSpinner && h2("vm-spinner", null), !this.noLoadingScreen && h2("vm-loading-screen", null), !this.noControls && h2("vm-default-controls", null), !this.noSettings && h2("vm-default-settings", null), h2("slot", null));
  }
  static get style() {
    return defaultUiCss;
  }
...
var Embed = class extends HTMLElement {
  ...
  render() {
    return h2("iframe", { id: this.id, class: "lazy", title: this.mediaTitle, "data-src": this.srcWithParams, allowFullScreen: true, allow: "autoplay; encrypted-media; picture-in-picture;", onLoad: this.onLoad.bind(this), ref: (el) => {
      this.iframe = el;
    } });
  }
  get host() {
...
var EndTime = class extends HTMLElement {
  ...
  render() {
    return h2("vm-time", { label: this.i18n.duration, seconds: Math.max(0, this.duration), alwaysShowHours: this.alwaysShowHours });
  }
  static get style() {
    return endTimeCss;
  }
...
var File = class extends HTMLElement {
  ...
  render() {
    ...
    const audio = h2("audio", Object.assign({ class: "lazy" }, mediaProps), h2("slot", null), "Your browser does not support the", h2("code", null, "audio"), "element.");
    const video = h2("video", Object.assign({ class: "lazy" }, mediaProps, {
      // eslint-disable-next-line @typescript-eslint/ban-ts-comment
      // @ts-ignore
      onenterpictureinpicture: this.onEnterPiP.bind(this),
      onleavepictureinpicture: this.onLeavePiP.bind(this)
    }), h2("slot", null), "Your browser does not support the", h2("code", null, "video"), "element.");
...
var FullscreenControl = class extends HTMLElement {
  ...
  render() {
    ...
    const tooltipWithHint = !isUndefined(this.keys) ? `${tooltip} (${this.keys})` : tooltip;
    return h2(Host, { hidden: !this.canSetFullscreen }, h2("vm-control", { label: this.i18n.fullscreen, keys: this.keys, pressed: this.isFullscreenActive, hidden: !this.canSetFullscreen, onClick: this.onClick.bind(this) }, h2("vm-icon", { name: this.isFullscreenActive ? this.exitIcon : this.enterIcon, library: this.icons }), h2("vm-tooltip", { hidden: this.hideTooltip, position: this.tooltipPosition, direction: this.tooltipDirection }, tooltipWithHint)));
  }
  static get watchers() {
    return {
      "playbackReady": ["onPlaybackReadyChange"]
...
var HLS = class extends HTMLElement {
  ...
  render() {
    return h2("vm-video", { willAttach: true, crossOrigin: this.crossOrigin, preload: this.preload, poster: this.poster, controlsList: this.controlsList, autoPiP: this.autoPiP, disablePiP: this.disablePiP, disableRemotePlayback: this.disableRemotePlayback, mediaTitle: this.mediaTitle, ref: (el) => {
      this.videoProvider = el;
    } }, h2("slot", null));
  }
};
...
var Icon = class extends HTMLElement {
  ...
  render() {
    return h2("div", { class: "icon", role: "img", "aria-label": this.getLabel(), innerHTML: this.svg });
  }
  static get watchers() {
    return {
      "name": ["handleChange"],
...
var LiveIndicator = class extends HTMLElement {
  ...
  render() {
    return h2("div", { class: {
      liveIndicator: true,
      hidden: !this.isLive
    } }, h2("div", { class: "indicator" }), this.i18n.live);
  }
  static get style() {
...
var LoadingScreen = class extends HTMLElement {
  ...
  render() {
    ...
    } }, h2("slot", null), !this.hideDots && h2("div", { class: "dotPulse" }));
  }
  static get style() {
...
var Menu = class extends HTMLElement {
  ...
  render() {
    var _a2, _b2, _c;
    return h2("div", { id: this.identifier, class: {
      menu: true,
...
var MenuItem = class extends HTMLElement {
  ...
  render() {
    ...
    return h2("div", { class: {
      ...
      this.menuItem = el;
    } }, showCheckedIcon && h2("vm-icon", { name: this.checkIcon, library: this.icons }), showLeftNavArrow && h2("span", { class: "arrow left" }), this.label, hasSpacer && h2("span", { class: "spacer" }), showHint && h2("span", { class: "hint" }, this.hint), showBadge && h2("span", { class: "badge" }, this.badge), showRightNavArrow && h2("span", { class: "arrow right" }));
  }
  get host() {
    return this;
  }
...
var MenuRadio = class extends HTMLElement {
  ...
  render() {
    return h2("vm-menu-item", { label: this.label, checked: this.checked, badge: this.badge, checkIcon: this.checkIcon, icons: this.icons, onClick: this.onClick.bind(this) });
  }
};
...
var MuteControl = class extends HTMLElement {
  ...
  render() {
    ...
    const tooltipWithHint = !isUndefined(this.keys) ? `${tooltip} (${this.keys})` : tooltip;
    return h2("vm-control", { label: this.i18n.mute, pressed: this.muted, keys: this.keys, onClick: this.onClick.bind(this) }, h2("vm-icon", { name: this.getIcon(), library: this.icons }), h2("vm-tooltip", { hidden: this.hideTooltip, position: this.tooltipPosition, direction: this.tooltipDirection }, tooltipWithHint));
  }
};
...
var PiPControl = class extends HTMLElement {
  ...
  render() {
    ...
    const tooltipWithHint = !isUndefined(this.keys) ? `${tooltip} (${this.keys})` : tooltip;
    return h2(Host, { hidden: !this.canSetPiP }, h2("vm-control", { label: this.i18n.pip, keys: this.keys, pressed: this.isPiPActive, hidden: !this.canSetPiP, onClick: this.onClick.bind(this) }, h2("vm-icon", { name: this.isPiPActive ? this.exitIcon : this.enterIcon, library: this.icons }), h2("vm-tooltip", { hidden: this.hideTooltip, position: this.tooltipPosition, direction: this.tooltipDirection }, tooltipWithHint)));
  }
  static get watchers() {
    return {
      "playbackReady": ["onPlaybackReadyChange"]
...
var PlaybackControl = class extends HTMLElement {
  ...
  render() {
    ...
    const tooltipWithHint = !isUndefined(this.keys) ? `${tooltip} (${this.keys})` : tooltip;
    return h2("vm-control", { label: this.i18n.playback, keys: this.keys, pressed: !this.paused, onClick: this.onClick.bind(this) }, h2("vm-icon", { name: this.paused ? this.playIcon : this.pauseIcon, library: this.icons }), h2("vm-tooltip", { hidden: this.hideTooltip, position: this.tooltipPosition, direction: this.tooltipDirection }, tooltipWithHint));
  }
};
var Logger = class {
  constructor() {
...
function withPlayerScheduler(player) {
  ...
  return function safeAdapterCall(prop, method2) {
    return __awaiter$9(this, void 0, void 0, function* () {
      ...
      const safeCall2 = (adapter) => __awaiter$9(this, void 0, void 0, function* () {
        var _a2;
        try {
          yield (_a2 = adapter === null || adapter === void 0 ? void 0 : adapter[method2]) === null || _a2 === void 0 ? void 0 : _a2.call(adapter, value);
        } catch (e) {
...
var Player = class extends HTMLElement {
  ...
  callAdapter(method2, value) {
    return __awaiter$8(this, void 0, void 0, function* () {
      return (yield this.adapter)[method2](value);
    });
  }
  hasCustomControls() {
  ...
  render() {
    ...
    return h2(Host, { id: this.genId(), idle: isIdle, mobile: this.isMobile, touch: this.isTouch, live: this.isLive, audio: this.isAudioView, video: this.isVideoView, pip: this.isPiPActive, fullscreen: this.isFullscreenActive }, h2("div", { "aria-label": label, "aria-hidden": !this.ready ? "true" : "false", "aria-busy": !this.playbackReady ? "true" : "false", class: {
      player: true,
      idle: isIdle,
      audio: this.isAudioView,
    ...
    } }, isBlockerVisible && h2("div", { class: "blocker" }), h2(Universe.Provider, { state: this.getPlayerState() }, h2("slot", null))));
  }
  get host() {
    return this;
  }
...
var Poster = class extends HTMLElement {
  ...
  render() {
    ...
    } }, h2("img", { class: "lazy", "data-src": this.currentPoster, alt: !isUndefined(this.mediaTitle) ? `${this.mediaTitle} Poster` : "Media Poster", style: { objectFit: this.fit }, onLoad: this.onPosterLoad.bind(this) }));
  }
  get host() {
    return this;
  }
...
var ScrubberControl = class extends HTMLElement {
  ...
  render() {
    ...
    return h2("div", { class: "scrubber", onMouseEnter: this.onSeeking.bind(this), onMouseLeave: this.onSeeking.bind(this), onMouseMove: this.onSeeking.bind(this), onTouchMove: () => {
      this.getSliderInput().focus();
    }, onTouchEnd: () => {
      this.getSliderInput().blur();
    } }, h2("vm-slider", { step: 0.01, max: this.endTime, value: this.currentTime, label: this.i18n.scrubber, valueText: sliderValueText, onVmValueChange: this.onSeek.bind(this), ref: (el) => {
      this.slider = el;
    } }), h2("progress", {
      class: {
    ...
    }, "% buffered"), h2("vm-tooltip", { hidden: this.hideTooltip, ref: (el) => {
      this.tooltip = el;
    } }, this.timestamp));
  }
  get host() {
...
var Settings = class extends HTMLElement {
  ...
  render() {
    return h2("div", { style: Object.assign({}, this.getPosition()), class: {
      settings: true,
      active: this.active,
      mobile: this.isMobile
    } }, h2("div", { class: "container", style: { height: `${this.menuHeight}px` } }, h2("vm-menu", { identifier: this.id, active: this.active, controller: this.controller, onVmOpen: this.onOpen.bind(this), onVmClose: this.onClose.bind(this), onVmMenuHeightChange: this.onHeightChange.bind(this), ref: (el) => {
      this.menu = el;
    } }, h2("slot", null))));
  }
  get host() {
...
var SettingsControl = class extends HTMLElement {
  ...
  render() {
    ...
    } }, h2("vm-control", { identifier: this.id, menu: this.menu, hidden: !hasSettings, expanded: this.expanded, label: this.i18n.settings, ref: (control) => {
      this.control = control;
    } }, h2("vm-icon", { name: this.icon, library: this.icons }), h2("vm-tooltip", { hidden: this.hideTooltip || this.expanded, position: this.tooltipPosition, direction: this.tooltipDirection }, this.i18n.settings)));
  }
  get host() {
    return this;
  }
...
var Submenu = class extends HTMLElement {
  ...
  render() {
    return h2("div", null, h2("vm-menu-item", { identifier: this.getControllerId(), menu: this.menu, label: this.label, hint: this.hint, expanded: this.active, ref: (el) => {
      writeTask(() => {
        this.controller = el;
      });
    } }), h2("vm-menu", { identifier: this.id, controller: this.controller, active: this.active, slideInDirection: this.slideInDirection, onVmOpen: this.onMenuOpen.bind(this), onVmClose: this.onMenuClose.bind(this), ref: (el) => {
      writeTask(() => {
        this.menu = el;
      });
    }, style: { top: `${this.getControllerHeightSync() + 1}px` } }, h2("slot", null)));
  }
  get host() {
    return this;
  }
...
var Time = class extends HTMLElement {
  ...
  render() {
    return h2("div", { class: "time", "aria-label": this.label }, formatTime(Math.max(0, this.seconds), this.alwaysShowHours));
  }
  static get style() {
    return timeCss;
  }
...
var TimeProgress = class extends HTMLElement {
  ...
  render() {
    return h2("div", { class: "timeProgress" }, h2("vm-current-time", { alwaysShowHours: this.alwaysShowHours }), h2("span", { class: "separator" }, this.separator), h2("vm-end-time", { alwaysShowHours: this.alwaysShowHours }));
  }
  static get style() {
...
var Tooltip = class extends HTMLElement {
  ...
  render() {
    return h2("div", { id: this.getId(), role: "tooltip", "aria-hidden": !this.active || this.isTouch || this.isMobile ? "true" : "false", class: {
      tooltip: true,
...
var VolumeControl = class extends HTMLElement {
  ...
  render() {
    return h2("div", { class: "volumeControl", onMouseEnter: this.onShowSlider.bind(this), onMouseLeave: this.onHideSlider.bind(this) }, h2("vm-mute-control", { keys: this.muteKeys, lowVolumeIcon: this.lowVolumeIcon, highVolumeIcon: this.highVolumeIcon, mutedIcon: this.mutedIcon, icons: this.icons, tooltipPosition: this.tooltipPosition, tooltipDirection: this.tooltipDirection, hideTooltip: this.hideTooltip, onVmFocus: this.onShowSlider.bind(this), onVmBlur: this.onHideSlider.bind(this) }), h2("vm-slider", { class: {
...
var VmSettingsControl = proxyCustomElement(SettingsControl, [1, "vm-settings-control", { "icon": [1], "icons": [1], "tooltipPosition": [1, "tooltip-position"], "tooltipDirection": [1, "tooltip-direction"], "menu": [1], "expanded": [4], "i18n": [16], "hideTooltip": [4, "hide-tooltip"], "vmSettings": [32] }]);
...
// node_modules/@vime/vue-next/dist/esm/lib.js
...
function listen2(node, event, handler) {
...
function render(tagName, events) {
  return function proxy() {
    ...
    return h(tagName, Object.assign(Object.assign({ ref: "ref" }, this.$props), {
      onVnodeMounted(vnode) {
        events.forEach((event) => {
          dispose.push(listen2(vnode.el, event, forwardEvent(event)));
        });
      },
...
// node_modules/@vime/vue-next/dist/esm/components/Audio.js
define("vm-audio", VmAudio);
define("vm-file", VmFile);
var Audio_default = defineComponent({
  emits: [],
  methods: {
    getAdapter: method("getAdapter")
  },
  render: render("vm-audio", [])
});

// node_modules/@vime/vue-next/dist/esm/components/CaptionControl.js
define("vm-caption-control", VmCaptionControl);
define("vm-control", VmControl);
define("vm-icon", VmIcon);
define("vm-tooltip", VmTooltip);
var CaptionControl_default = defineComponent({
  emits: [],
  methods: {},
  render: render("vm-caption-control", [])
});

// node_modules/@vime/vue-next/dist/esm/components/Captions.js
define("vm-captions", VmCaptions);
var Captions_default = defineComponent({
  emits: [],
  methods: {},
  render: render("vm-captions", [])
});

// node_modules/@vime/vue-next/dist/esm/components/ClickToPlay.js
define("vm-click-to-play", VmClickToPlay);
var ClickToPlay_default = defineComponent({
  emits: [],
  methods: {
    forceClick: method("forceClick")
  },
  render: render("vm-click-to-play", [])
});

// node_modules/@vime/vue-next/dist/esm/components/Control.js
define("vm-control", VmControl);
var Control_default = defineComponent({
  emits: ["vmInteractionChange", "vmFocus", "vmBlur"],
  methods: {
    focusControl: method("focusControl"),
    blurControl: method("blurControl")
  },
  render: render("vm-control", ["vmInteractionChange", "vmFocus", "vmBlur"])
});

// node_modules/@vime/vue-next/dist/esm/components/ControlGroup.js
define("vm-control-group", VmControlGroup);
var ControlGroup_default = defineComponent({
  emits: [],
  methods: {},
  render: render("vm-control-group", [])
});

// node_modules/@vime/vue-next/dist/esm/components/ControlSpacer.js
define("vm-control-spacer", VmControlSpacer);
var ControlSpacer_default = defineComponent({
  emits: [],
  methods: {},
  render: render("vm-control-spacer", [])
});

// node_modules/@vime/vue-next/dist/esm/components/Controls.js
define("vm-controls", VmControls);
var Controls_default = defineComponent({
  emits: [],
  methods: {},
  render: render("vm-controls", [])
});

// node_modules/@vime/vue-next/dist/esm/components/CurrentTime.js
define("vm-current-time", VmCurrentTime);
define("vm-time", VmTime);
var CurrentTime_default = defineComponent({
  emits: [],
  methods: {},
  render: render("vm-current-time", [])
});

// node_modules/@vime/vue-next/dist/esm/components/Dailymotion.js
define("vm-dailymotion", VmDailymotion);
define("vm-embed", VmEmbed);
var Dailymotion_default = defineComponent({
  emits: ["vmLoadStart", "vmError"],
  methods: {
    getAdapter: method("getAdapter")
  },
  render: render("vm-dailymotion", ["vmLoadStart", "vmError"])
});

// node_modules/@vime/vue-next/dist/esm/components/Dash.js
define("vm-dash", VmDash);
define("vm-file", VmFile);
define("vm-video", VmVideo);
var Dash_default = defineComponent({
  emits: ["vmLoadStart", "vmError"],
  methods: {
    getAdapter: method("getAdapter")
  },
  render: render("vm-dash", ["vmLoadStart", "vmError"])
});

// node_modules/@vime/vue-next/dist/esm/components/DblClickFullscreen.js
define("vm-dbl-click-fullscreen", VmDblClickFullscreen);
var DblClickFullscreen_default = defineComponent({
  emits: [],
  methods: {},
  render: render("vm-dbl-click-fullscreen", [])
});

// node_modules/@vime/vue-next/dist/esm/components/DefaultControls.js
...
var DefaultControls_default = defineComponent({
  emits: [],
  methods: {},
  render: render("vm-default-controls", [])
});

// node_modules/@vime/vue-next/dist/esm/components/DefaultSettings.js
...
var DefaultSettings_default = defineComponent({
  emits: [],
  methods: {},
  render: render("vm-default-settings", [])
});

// node_modules/@vime/vue-next/dist/esm/components/DefaultUi.js
...
define("vm-ui", VmUi);
var DefaultUi_default = defineComponent({
  emits: [],
  methods: {},
  render: render("vm-default-ui", [])
});

// node_modules/@vime/vue-next/dist/esm/components/Embed.js
define("vm-embed", VmEmbed);
var Embed_default = defineComponent({
  emits: ["vmEmbedSrcChange", "vmEmbedMessage", "vmEmbedLoaded"],
  methods: {
    postMessage: method("postMessage")
  },
  render: render("vm-embed", ["vmEmbedSrcChange", "vmEmbedMessage", "vmEmbedLoaded"])
});

// node_modules/@vime/vue-next/dist/esm/components/EndTime.js
define("vm-end-time", VmEndTime);
define("vm-time", VmTime);
var EndTime_default = defineComponent({
  emits: [],
  methods: {},
  render: render("vm-end-time", [])
});

// node_modules/@vime/vue-next/dist/esm/components/File.js
define("vm-file", VmFile);
var File_default = defineComponent({
  emits: ["vmLoadStart", "vmError", "vmMediaElChange", "vmSrcSetChange"],
  methods: {
    getAdapter: method("getAdapter")
  },
  render: render("vm-file", ["vmLoadStart", "vmError", "vmMediaElChange", "vmSrcSetChange"])
});

// node_modules/@vime/vue-next/dist/esm/components/FullscreenControl.js
define("vm-fullscreen-control", VmFullscreenControl);
define("vm-control", VmControl);
define("vm-icon", VmIcon);
define("vm-tooltip", VmTooltip);
var FullscreenControl_default = defineComponent({
  emits: [],
  methods: {},
  render: render("vm-fullscreen-control", [])
});

// node_modules/@vime/vue-next/dist/esm/components/Hls.js
define("vm-hls", VmHls);
define("vm-file", VmFile);
define("vm-video", VmVideo);
var Hls_default = defineComponent({
  emits: ["vmLoadStart", "vmError"],
  methods: {
    getAdapter: method("getAdapter")
  },
  render: render("vm-hls", ["vmLoadStart", "vmError"])
});

// node_modules/@vime/vue-next/dist/esm/components/Icon.js
define("vm-icon", VmIcon);
var Icon_default = defineComponent({
  ...
  render: render("vm-icon", ["vmLoad", "vmError"])
});

// node_modules/@vime/vue-next/dist/esm/components/IconLibrary.js
define("vm-icon-library", VmIconLibrary);
var IconLibrary_default = defineComponent({
  emits: [],
  methods: {},
  render: render("vm-icon-library", [])
});

// node_modules/@vime/vue-next/dist/esm/components/LiveIndicator.js
define("vm-live-indicator", VmLiveIndicator);
var LiveIndicator_default = defineComponent({
  emits: [],
  methods: {},
  render: render("vm-live-indicator", [])
});

// node_modules/@vime/vue-next/dist/esm/components/LoadingScreen.js
define("vm-loading-screen", VmLoadingScreen);
var LoadingScreen_default = defineComponent({
  emits: [],
  methods: {},
  render: render("vm-loading-screen", [])
});

// node_modules/@vime/vue-next/dist/esm/components/Menu.js
define("vm-menu", VmMenu);
var Menu_default = defineComponent({
  emits: ["vmOpen", "vmClose", "vmFocus", "vmBlur", "vmActiveSubmenuChange", "vmActiveMenuItemChange", "vmMenuHeightChange"],
  methods: {
    focusMenu: method("focusMenu"),
    blurMenu: method("blurMenu"),
    getActiveMenuItem: method("getActiveMenuItem"),
    setActiveMenuItem: method("setActiveMenuItem"),
    calculateHeight: method("calculateHeight")
  },
  render: render("vm-menu", ["vmOpen", "vmClose", "vmFocus", "vmBlur", "vmActiveSubmenuChange", "vmActiveMenuItemChange", "vmMenuHeightChange"])
});

// node_modules/@vime/vue-next/dist/esm/components/MenuItem.js
define("vm-menu-item", VmMenuItem);
define("vm-icon", VmIcon);
var MenuItem_default = defineComponent({
  emits: ["vmFocus", "vmBlur"],
  methods: {
    focusItem: method("focusItem"),
    blurItem: method("blurItem"),
    getHeight: method("getHeight")
  },
  render: render("vm-menu-item", ["vmFocus", "vmBlur"])
});

// node_modules/@vime/vue-next/dist/esm/components/MenuRadio.js
define("vm-menu-radio", VmMenuRadio);
define("vm-icon", VmIcon);
define("vm-menu-item", VmMenuItem);
var MenuRadio_default = defineComponent({
  emits: ["vmCheck"],
  methods: {},
  render: render("vm-menu-radio", ["vmCheck"])
});

// node_modules/@vime/vue-next/dist/esm/components/MenuRadioGroup.js
define("vm-menu-radio-group", VmMenuRadioGroup);
var MenuRadioGroup_default = defineComponent({
  emits: ["vmCheck"],
  methods: {},
  render: render("vm-menu-radio-group", ["vmCheck"])
});

// node_modules/@vime/vue-next/dist/esm/components/MuteControl.js
define("vm-mute-control", VmMuteControl);
define("vm-control", VmControl);
define("vm-icon", VmIcon);
define("vm-tooltip", VmTooltip);
var MuteControl_default = defineComponent({
  emits: ["vmFocus", "vmBlur"],
  methods: {},
  render: render("vm-mute-control", ["vmFocus", "vmBlur"])
});

// node_modules/@vime/vue-next/dist/esm/components/PipControl.js
define("vm-pip-control", VmPipControl);
define("vm-control", VmControl);
define("vm-icon", VmIcon);
define("vm-tooltip", VmTooltip);
var PipControl_default = defineComponent({
  emits: [],
  methods: {},
  render: render("vm-pip-control", [])
});

// node_modules/@vime/vue-next/dist/esm/components/PlaybackControl.js
define("vm-playback-control", VmPlaybackControl);
define("vm-control", VmControl);
define("vm-icon", VmIcon);
define("vm-tooltip", VmTooltip);
var PlaybackControl_default = defineComponent({
  emits: [],
  methods: {},
  render: render("vm-playback-control", [])
});

// node_modules/@vime/vue-next/dist/esm/components/Player.js
define("vm-player", VmPlayer);
var Player_default = defineComponent({
...
// node_modules/@vime/vue-next/dist/esm/components/Poster.js
define("vm-poster", VmPoster);
var Poster_default = defineComponent({
  emits: ["vmLoaded", "vmWillShow", "vmWillHide"],
  methods: {},
  render: render("vm-poster", ["vmLoaded", "vmWillShow", "vmWillHide"])
});

// node_modules/@vime/vue-next/dist/esm/components/Scrim.js
define("vm-scrim", VmScrim);
var Scrim_default = defineComponent({
  emits: [],
  methods: {},
  render: render("vm-scrim", [])
});

// node_modules/@vime/vue-next/dist/esm/components/ScrubberControl.js
define("vm-scrubber-control", VmScrubberControl);
define("vm-slider", VmSlider);
define("vm-tooltip", VmTooltip);
var ScrubberControl_default = defineComponent({
  emits: [],
  methods: {},
  render: render("vm-scrubber-control", [])
});

// node_modules/@vime/vue-next/dist/esm/components/Settings.js
define("vm-settings", VmSettings);
define("vm-menu", VmMenu);
var Settings_default = defineComponent({
  emits: [],
  methods: {
    setController: method("setController")
  },
  render: render("vm-settings", [])
});

// node_modules/@vime/vue-next/dist/esm/components/SettingsControl.js
define("vm-settings-control", VmSettingsControl);
define("vm-control", VmControl);
define("vm-icon", VmIcon);
define("vm-tooltip", VmTooltip);
var SettingsControl_default = defineComponent({
  emits: [],
  methods: {
    focusControl: method("focusControl"),
    blurControl: method("blurControl")
  },
  render: render("vm-settings-control", [])
});

// node_modules/@vime/vue-next/dist/esm/components/Skeleton.js
define("vm-skeleton", VmSkeleton);
var Skeleton_default = defineComponent({
  emits: [],
  methods: {},
  render: render("vm-skeleton", [])
});

// node_modules/@vime/vue-next/dist/esm/components/Slider.js
define("vm-slider", VmSlider);
var Slider_default = defineComponent({
  emits: ["vmValueChange", "vmFocus", "vmBlur"],
  methods: {},
  render: render("vm-slider", ["vmValueChange", "vmFocus", "vmBlur"])
});

// node_modules/@vime/vue-next/dist/esm/components/Spinner.js
define("vm-spinner", VmSpinner);
var Spinner_default = defineComponent({
  emits: ["vmWillShow", "vmWillHide"],
  methods: {},
  render: render("vm-spinner", ["vmWillShow", "vmWillHide"])
});

// node_modules/@vime/vue-next/dist/esm/components/Submenu.js
define("vm-submenu", VmSubmenu);
define("vm-icon", VmIcon);
define("vm-menu", VmMenu);
define("vm-menu-item", VmMenuItem);
var Submenu_default = defineComponent({
  emits: ["vmOpenSubmenu", "vmCloseSubmenu"],
  methods: {
    getController: method("getController"),
    getMenu: method("getMenu"),
    getControllerHeight: method("getControllerHeight")
  },
  render: render("vm-submenu", ["vmOpenSubmenu", "vmCloseSubmenu"])
});

// node_modules/@vime/vue-next/dist/esm/components/Time.js
define("vm-time", VmTime);
var Time_default = defineComponent({
  emits: [],
  methods: {},
  render: render("vm-time", [])
});

// node_modules/@vime/vue-next/dist/esm/components/TimeProgress.js
define("vm-time-progress", VmTimeProgress);
define("vm-current-time", VmCurrentTime);
define("vm-time", VmTime);
define("vm-end-time", VmEndTime);
var TimeProgress_default = defineComponent({
  emits: [],
  methods: {},
  render: render("vm-time-progress", [])
});

// node_modules/@vime/vue-next/dist/esm/components/Tooltip.js
define("vm-tooltip", VmTooltip);
var Tooltip_default = defineComponent({
  emits: [],
  methods: {},
  render: render("vm-tooltip", [])
});

// node_modules/@vime/vue-next/dist/esm/components/Ui.js
define("vm-ui", VmUi);
var Ui_default = defineComponent({
  emits: [],
  methods: {},
  render: render("vm-ui", [])
});

// node_modules/@vime/vue-next/dist/esm/components/Video.js
define("vm-video", VmVideo);
define("vm-file", VmFile);
var Video_default = defineComponent({
  emits: [],
  methods: {
    getAdapter: method("getAdapter")
  },
  render: render("vm-video", [])
});

// node_modules/@vime/vue-next/dist/esm/components/Vimeo.js
define("vm-vimeo", VmVimeo);
define("vm-embed", VmEmbed);
var Vimeo_default = defineComponent({
  emits: ["vmLoadStart", "vmError"],
  methods: {
    getAdapter: method("getAdapter")
  },
  render: render("vm-vimeo", ["vmLoadStart", "vmError"])
});

// node_modules/@vime/vue-next/dist/esm/components/VolumeControl.js
define("vm-volume-control", VmVolumeControl);
define("vm-control", VmControl);
define("vm-icon", VmIcon);
define("vm-mute-control", VmMuteControl);
define("vm-tooltip", VmTooltip);
define("vm-slider", VmSlider);
var VolumeControl_default = defineComponent({
  emits: [],
  methods: {},
  render: render("vm-volume-control", [])
});

// node_modules/@vime/vue-next/dist/esm/components/Youtube.js
...
var Youtube_default = defineComponent({
  ...
  render: render("vm-youtube", ["vmLoadStart"])
});

// node_modules/@vime/vue-next/dist/esm/components/index.js
...
// node_modules/@vime/vue-next/dist/esm/composition.js
...
var usePlayer = (el) => {
  ...
  const find = () => __awaiter2(void 0, void 0, void 0, function* () {
    var _c;
...
var usePlayerContext2 = (el, prop, defaultValue) => {
  const noop2 = () => {
  };
  const binding = ref(defaultValue);
  const dispatch = ref(noop2);
  let prevValue = defaultValue;
  watch(el, () => {
    dispatch.value = el.value ? createDispatcher(el.value) : noop2;
  });
  watch(binding, () => {
    if (binding.value !== prevValue)
      dispatch.value(prop, binding.value);
  });
  watch(el, (_a2, _b2, onInvalidate) => __awaiter2(void 0, void 0, void 0, function* () {
    if (!el.value)
      return;
    const off = yield usePlayerContext(el.value, [prop], (_, newValue) => {
...
//# sourceMappingURL=@vime_vue-next.js.map
